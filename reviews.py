...
sql = "INSERT IGNORE INTO dashboard_reviews (id , product_id  , date_created_gmt ,  status ,  reviewer ,  reviewer_email ,  review , rating ,  verified	) VALUES ( %s  , %s ,%s , %s ,%s , %s ,%s, %s ,%s)"
...
while (n < m) and ( x != None) :
    n += 1
    AllOrders = wcapi.get("products/reviews", params = {'per_page' : x , 'page': n }).json() 
    for x_Dict in AllOrders:

      val = ( x_Dict['id'],  x_Dict['product_id'], x_Dict['date_created_gmt'],x_Dict['status'],x_Dict['reviewer'],x_Dict['reviewer_email'],x_Dict['review'],x_Dict['rating'],x_Dict['verified'],)
      db.execute(sql, val)
      mydb.commit()
