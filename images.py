...
sql = "INSERT IGNORE INTO dashboard_Reviews (id , date_created_gmt ,src , date_modified_gmt	) VALUES ( %s ,%s , %s ,%s)"
...
while (n < m) and ( x != None) :
    n += 1
    AllOrders = wcapi.get("products/images", params = {'per_page' : x , 'page': n }).json() 
    for x_Dict in AllOrders:

      val = ( x_Dict['id'], x_Dict['name'], x_Dict['slug'], x_Dict['slug'])
      db.execute(sql, val)
      mydb.commit()
